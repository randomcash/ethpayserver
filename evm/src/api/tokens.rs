...
pub async fn list_tokens<D, A>(
    _admin: AdminAuth,
    State(state): State<EvmState<D, A>>,
...
pub async fn set_token_enabled<D, A>(
    _admin: AdminAuth,
    State(state): State<EvmState<D, A>>,
    Path(id): Path<i64>,
