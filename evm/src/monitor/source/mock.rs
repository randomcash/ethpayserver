...
struct Inner {
    ...
    ///
    /// Cost here is not a performance nicety. Native payment detection polls
    /// one balance per watched address per block, so a single open invoice on
    /// a 2-second chain is over a million requests a month - and the shape of
    /// that, calls-per-block against addresses-watched, is invisible to every
    /// test that checks only whether a payment was found.
