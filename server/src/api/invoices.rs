...
mod tests {
    ...
    fn make_entry(chain_id: i64, token_address: Option<&str>) -> types::StoreTokenPolicyEntry {
        types::StoreTokenPolicyEntry {
            id: uuid::Uuid::new_v4(),
    ...
    fn make_policy(
        mode: types::TokenPolicyMode,
        entries: Vec<types::StoreTokenPolicyEntry>,
    ...
    fn test_token_policy_allowlist_filters_correctly() {
        ...
        let policy = make_policy(types::TokenPolicyMode::Allowlist, vec![make_entry(1, None)]);
    ...
    fn test_token_policy_blocklist_filters_correctly() {
        ...
        let policy = make_policy(
            types::TokenPolicyMode::Blocklist,
            vec![make_entry(1, Some(usdc))],
        );
        apply_token_policy_filter(&mut methods, &policy);
    ...
    fn test_token_policy_allowlist_empty_entries_blocks_all() {
        ...
        let policy = make_policy(types::TokenPolicyMode::Allowlist, vec![]);
        apply_token_policy_filter(&mut methods, &policy);
    ...
    fn test_token_policy_blocklist_empty_entries_allows_all() {
        let mut methods = vec![make_pm(1, None), make_pm(137, None)];
        let policy = make_policy(types::TokenPolicyMode::Blocklist, vec![]);
        apply_token_policy_filter(&mut methods, &policy);
        assert_eq!(methods.len(), 2);
    }
}
