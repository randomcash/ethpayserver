...
mod tests {
    ...
    fn a_wallet_registered_from_derive_xpub_output_verifies_against_its_own_addresses() {
        ...
        let xpub = wallet.account_xpub_string_for(ChainFamily::Evm).unwrap();
        ...
        let stored_wallet = data_service::Wallet {
            id: Uuid::new_v4(),
            user_id: Uuid::new_v4(),
        ...
        let entries = derive_entries(&stored_wallet, 0, VERIFICATION_ADDRESS_COUNT).unwrap();

        let actual: Vec<String> = entries.into_iter().map(|entry| entry.address).collect();
        assert_eq!(
            actual, expected,
            "verification_addresses diverged from what derive-xpub printed for the same key"
        );
    }
}
