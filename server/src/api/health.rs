    ...
    A: Send + Sync + 'static,
{
    ...
    let pg_result = tokio::time::timeout(PROBE_TIMEOUT, state.data_service.health_check()).await;
    ...
    // Check Redis + RPC chains
    let (redis, rpcs, monitor) = if let Some(ref evm_monitor) = state.evm_monitor {
        let redis_start = Instant::now();
        let redis_result = tokio::time::timeout(PROBE_TIMEOUT, evm_monitor.health_check()).await;
        ...
        // Fetch chain health from Redis
        let chains_start = Instant::now();
        let chains_result =
            tokio::time::timeout(PROBE_TIMEOUT, evm_monitor.get_chain_health()).await;
        let chains_latency = chains_start.elapsed().as_millis() as u64;

        let mut rpcs = HashMap::new();
        let mut data_fresh = false;

        match chains_result {
            Ok(Ok(chains)) => {
                // If we got data, the monitor is publishing
                data_fresh = !chains.is_empty();
                for chain in chains {
