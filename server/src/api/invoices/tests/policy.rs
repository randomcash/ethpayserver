...
use super::super::*;
use ::types::{ChainId, StoreTokenPolicyEntry, TokenPolicyMode};

fn make_pm(chain_id: ChainId, token_address: Option<&str>) -> data_service::StorePaymentMethod {
...
fn make_entry(chain_id: ChainId, token_address: Option<&str>) -> StoreTokenPolicyEntry {
...
fn test_token_policy_allowlist_filters_correctly() {
    let mut methods = vec![
        make_pm(ChainId::evm(1), None),
        make_pm(
            ChainId::evm(1),
            Some("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"),
        ),
        make_pm(ChainId::evm(137), None),
    ];
    let policy = make_policy(
        TokenPolicyMode::Allowlist,
        vec![make_entry(ChainId::evm(1), None)],
    );
    apply_token_policy_filter(&mut methods, &policy);
    assert_eq!(methods.len(), 1);
    assert_eq!(methods[0].chain_id, ChainId::evm(1));
...
fn test_token_policy_blocklist_filters_correctly() {
    ...
    let mut methods = vec![
        make_pm(ChainId::evm(1), None),
        make_pm(ChainId::evm(1), Some(usdc)),
        make_pm(ChainId::evm(137), None),
    ];
    let policy = make_policy(
        TokenPolicyMode::Blocklist,
        vec![make_entry(ChainId::evm(1), Some(usdc))],
    );
    apply_token_policy_filter(&mut methods, &policy);
...
fn test_token_policy_native_asset_handling() {
    ...
    let mut methods = vec![
        make_pm(ChainId::evm(1), None),
        make_pm(ChainId::evm(1), Some(usdt)),
    ];
    let policy = make_policy(
        TokenPolicyMode::Allowlist,
        vec![make_entry(ChainId::evm(1), None)],
