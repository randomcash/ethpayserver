...
use types::{
    InvoiceId, InvoiceQueryParams, InvoiceReader, InvoiceStatus, InvoiceWriter, PaymentOptionId,
    PaymentQueryParams, PaymentReader, PaymentWriter, StoreId, WatchedAddressReader,
...
async fn invoice_search_filters_the_count_and_the_page_together() {
    ...
    let (total, rows) = InvoiceReader::query(&ds, &InvoiceQueryParams::new().with_search("eur"))
        .await
        .unwrap();
...
async fn payment_search_covers_hash_symbol_and_sender_and_respects_scope() {
    ...
    for term in ["eth", "cdef1234"] {
        let (total, _) = PaymentReader::query(&ds, &PaymentQueryParams::new().with_search(term))
            .await
            .unwrap();
